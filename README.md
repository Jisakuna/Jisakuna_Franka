...
以下信息来自项目已有的[环境记录](catkin_ws/src/panda_rt_tools/README.md)，不表示本次发布已重新完成硬件验证。
...
在 [installation-assets-v1 Release](https://github.com/Jisakuna/FRANKA_INSTALL/releases/tag/installation-assets-v1) 下载以下文件，放在仓库根目录：
...
原环境配置详情见[包内 README](catkin_ws/src/panda_rt_tools/README.md)。程序尝试设置 `SCHED_FIFO` 优先级 90，并调用 `mlockall`。控制主机应具备 PREEMPT_RT 内核、适当的 `rtprio` / `memlock` 权限和稳定的 FCI 有线链路。
